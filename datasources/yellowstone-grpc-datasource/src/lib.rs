                    ...
                    match result {
                        Ok((mut subscribe_tx, mut stream)) => {
                            while let Some(result) = stream.next().await {
                                if shutdown_recv.is_shutdown_now() {
                                    break;
                                }
                                    ...
                                    Ok(update) => match update.update_oneof {
                                        ...
                                        Some(UpdateOneof::Ping(_)) => {
                                            if let Err(error) = subscribe_tx
                                                ...
                                                .await
                                            {
                                                log::error!("Failed to send ping (id: {datasource_id}): {error}");
                                                break;
                                            }
                                        }
...
fn send_subscribe_account_update(
    ...
    if let Some(account_info) = subscribe_update_account.account {
        ...
        let Some(raw_signature) = account_info.txn_signature else {
            return;
        };

        let Ok(signature) = solana_signature::Signature::try_from(raw_signature.as_slice()) else {
            return;
        };
